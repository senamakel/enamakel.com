        ...
        <li key={item.title} className="border-border border-b last:border-b-0">
          ...
          >
            <span className="min-w-0 flex-1">
              <span className="border-foreground/45 group-hover:border-foreground border-b-2 border-dotted transition-colors inline-block font-sans">
                {item.title}
              </span>
    ...
    <ul className="mt-4">
      {items.map((item) => (
        <li key={item.href}>
          ...
          >
            <span className="min-w-0 flex-1">
              <span className="border-foreground/45 group-hover:border-foreground border-b-2 border-dotted transition-colors font-sans">{item.title}</span>
            </span>
            <span className={`${LABEL} shrink-0 tabular-nums`}>
    ...
    <div className="mx-auto w-full max-w-[1440px] px-6 sm:px-10 lg:px-16">
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-x-8">
        ...
        <aside className="border-border flex flex-col gap-6 py-10 lg:col-span-3 lg:sticky lg:top-0 lg:h-screen lg:max-w-[16rem] lg:self-start lg:py-16">
          <nav className="flex flex-col gap-1 font-sans">
            {site.nav.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="border-foreground/45 hover:border-foreground border-b-2 border-dotted transition-colors w-fit"
              >
                {item.label}
              </a>
            ))}
          ...
          <HexTile className="hidden lg:block lg:flex-1" />
        </aside>
